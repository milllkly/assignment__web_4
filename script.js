...
cards.forEach(function(card) {
    card.addEventListener('mouseover', function() {
        this.classList.add('shadow-lg'); // Bootstrap class for larger shadow
    });
...
;
const flowerShop = {
    name: "Gulder delivery",
    featuredFlowers: ["Cloves", "Spray roses", "Gladioli"],
    order: function (name, email, flowerType, message) {
        // Process the order
        console.log(`Order received for ${flowerType} by ${name}`);
    }
};
document.addEventListener("DOMContentLoaded", function() {
    ...
    const orderButton = document.querySelector('.btn-custom');
    orderButton.addEventListener('click', function(event) {
        event.preventDefault(); // Prevent form submission for demo purposes
        // Set the tooltip message
        $(this).attr('title', 'Wow! You have a -10% bonus.');
        $(this).tooltip('show');
        
        // Optionally hide the tooltip after a delay
        setTimeout(() => $(this).tooltip('hide'), 3000); // hides after 3 seconds
    });
});

document.addEventListener('DOMContentLoaded', () => {
    const box = document.getElementById('animatedElement');
    const trigger = document.getElementById('triggerAnimation');

    trigger.addEventListener('click', () => {
        fadeIn(box, 1000); // Fades in over 1000 milliseconds
    });
});
